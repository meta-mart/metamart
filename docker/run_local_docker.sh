...
if [[ $skipMaven == "false" ]]; then
    if [[ $mode == "no-ui" ]]; then
        ...
        mvn -DskipTests -DonlyBackend clean package -pl !metamart-ui
    else
...
until curl -s -f "http://localhost:9200/_cat/indices/metamart_team_search_index"; do
...
tput setaf 2
echo "✔ MetaMart is up and running"
