...
# MetaMart env script
. $CATALOG_HOME/conf/metamart-env.sh

function catalogStart {
   catalogStatus -q
   if [[ $? -eq 0 ]]; then
       rm -f ${PID_FILE}
       echo "Starting MetaMart"
       ...
       echo "MetaMart already running with PID: ${PID}"
   fi
}
...
function catalogStop {
   catalogStatus -q
   if [[ $? -eq 1 ]]; then
       echo "Stopping MetaMart [${PID}] "
       kill -s KILL ${PID} 1>>"${OUT_FILE}" 2>>"${ERR_FILE}"
   else
       echo "MetaMart not running"
   fi
}
...
function catalogStatus {
   ...
   if [[ $? -eq 1 ]] || [[ ${PID} -eq 0 ]]; then
     [[ "${verbose}" -eq 1 ]] && echo "MetaMart not running."
     return 0
   fi

   ps -p ${PID} > /dev/null
   if [[ $? -eq 0 ]]; then
     [[ "${verbose}" -eq 1 ]] && echo "MetaMart running with PID=${PID}."
     return 1
   else
     [[ "${verbose}" -eq 1 ]] && echo "MetaMart not running."
     return 0
...
# Removes the PID file if MetaMart is not running
function catalogClean {
   ...
   if [[ $? -eq 0 ]]; then
     ...
     echo "Can't clean files. MetaMart running with PID=${PID}."
   fi
}

# Returns 0 if the MetaMart is running and sets the $PID variable.
function getPID {
   if [ ! -d $PID_DIR ]; then
...
case "${option}" in
  start)
     conf="$CATALOG_HOME/conf/metamart.yaml"
