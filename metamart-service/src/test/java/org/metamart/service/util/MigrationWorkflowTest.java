...
public class MigrationWorkflowTest extends MetaMartApplicationTest {

  public static MigrationWorkflow migrationWorkflow;
  public static List<MigrationFile> omMigrationList;
  public static List<MigrationFile> digitransMigrationList;
  ...
  public static void setup() {
    ...
    digitransMigrationList =
        List.of(
            new MigrationFile(
                new File("/bootstrap-digitrans/sql/migrations/native/1.1.0-digitrans"),
                null,
                ConnectionType.MYSQL,
                migrationWorkflow.getPipelineServiceClientConfiguration(),
                migrationWorkflow.getAuthenticationConfiguration(),
                true),
            new MigrationFile(
                new File("/bootstrap-digitrans/sql/migrations/native/1.2.2-digitrans"),
                null,
                ConnectionType.MYSQL,
                migrationWorkflow.getPipelineServiceClientConfiguration(),
                migrationWorkflow.getAuthenticationConfiguration(),
  ...
  void test_getMigrationFiles() {
    ...
    Mockito.doReturn(digitransMigrationList)
        .when(migrationWorkflow)
        .getMigrationFilesFromPath(
    ...
    assertEquals(
        List.of("1.1.0", "1.1.0-digitrans", "1.2.0", "1.2.1", "1.2.2-digitrans"),
  ...
  void test_getMigrationsToApply() {
    ...
    availableMigrations.addAll(digitransMigrationList);

    // If we only have executed native migrations, we'll execute the Digitrans ones
    assertEquals(
        List.of("1.1.0-digitrans", "1.2.2-digitrans"),
        migrationWorkflow
    ...
    assertEquals(
        List.of("1.2.1", "1.2.2-digitrans"),
        migrationWorkflow
            .getMigrationsToApply(List.of("1.1.0", "1.1.0-digitrans", "1.2.0"), availableMigrations)
            .stream()
            .map(f -> f.dir.getName())
            .collect(Collectors.toList()));
  }
