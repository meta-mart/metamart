...
import static org.metamart.service.apps.scheduler.AppScheduler.APPS_JOB_GROUP;
import static org.metamart.service.apps.scheduler.AppScheduler.APP_INFO_KEY;
...
import org.metamart.schema.api.configuration.apps.AppPrivateConfig;
...
public class ApplicationHandler {
  ...
  private final MetaMartApplicationConfig config;
  private final AppRepository appRepository;
  private final ConfigurationReader configReader = new ConfigurationReader();

  private ApplicationHandler(MetaMartApplicationConfig config) {
    this.config = config;
  ...
  public void setAppRuntimeProperties(App app) {
    app.setMetaMartServerConnection(
        new MetaMartConnectionBuilder(config, app.getBot().getName()).build());
    try {
  ...
  public void migrateQuartzConfig(App application) throws SchedulerException {
    ...
    updatedApp.setMetaMartServerConnection(null);
