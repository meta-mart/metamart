package org.metamart.service.migration.utils;
...
public class MigrationFile implements Comparable<MigrationFile> {
  ...
  public static final String DEFAULT_MIGRATION_PROCESS_CLASS =
      "org.metamart.service.migration.api.MigrationProcessImpl";
  ...
  public String getMigrationProcessClassName() {
    String clazzName =
        String.format(
            "org.metamart.service.migration.%s.%s.Migration",
  ...
  public String getMigrationProcessExtClassName() {
    String clazzName =
        String.format(
            "io.digitrans.service.migration.%s.%s.Migration", dbPackageName, getVersionPackageName());
    try {
      Class.forName(clazzName);
    } catch (ClassNotFoundException e) {
      return null;
