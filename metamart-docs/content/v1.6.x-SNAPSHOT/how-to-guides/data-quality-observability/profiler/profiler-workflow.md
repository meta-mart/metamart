...
Configuration to automatically tag columns that might contain sensitive information. PII data will be infered from the column name. If `Generate Sample Data` is toggled on MetaMart will leverage machine learning to infer which column may contain PII sensitive data.

**Advanced Configuration**  

**PII Inference Confidence LevelConfidence (Optional)**  
If `Auto PII Tagging` is enable, this confidence level will determine the threshold to use for MetaMart's NLP model to consider a column as containing PII data.
...
You can find all the definitions and types for the  `sourceConfig` [here](https://github.com/meta-mart/MetaMart/blob/main/metamart-spec/src/main/resources/json/schema/metadataIngestion/databaseServiceProfilerPipeline.json).
