...
MetaMart has been providing alerts right from the start to notify users of important data lifecycle events: schema modifications, ownership shifts, and tagging updates. Users can define fine-grained alerts and notifications.
...
- **Ingestion Pipeline Monitoring:** for MetaMart’s ingestion workflows

Depending on your use cases, notifications can be sent to owners, admins, teams, or users, providing a more personalized and informed experience. Teams can configure their dedicated Slack, MS Teams, or Google Chat channels to receive notifications related to their data assets, streamlining communication and collaboration. With the alerts and notifications in MetaMart, users can send Announcements over email, Slack, or Teams. Alerts are sent to a user when they are mentioned in a task or an activity feed.
...
As with most configuration in MetaMart you will first need to add a **Name** and **Description** for your alert. Once this is completed, we can move on to the configuration. You have 4 steps to configure your alert:
...
4. **Destination**: Specify the destination for the alerts. MetaMart supports sending alerts to the users within MetaMart, i.e., **Internal** such as Admins, Followers, Owners, Teams and Users.

{% image
src="/images/v1.5/how-to-guides/admin-guide/internal.webp"
alt="Send Alerts to the Users and Teams within MetaMart"
caption="Send Alerts to the Users and Teams within MetaMart"
/%}
...
3. **Destination**: Specify the destination for the notifications. MetaMart supports sending notifications to the users within MetaMart, i.e., **Internal** such as Admins, Followers, Owners, Teams and Users. Notifications can be sent to **External** sources like Email, G Chat, Generic Webhooks, MS Teams, and Slack. Add the endpoint URL for the destinations. Notifications can be sent to multiple destinations.

## Additional Details for the Configuration of Destinations

### Email
To enable email alerts you will need to ensure that you have an SMTP server available. With the information for your SMTP server you can configure MetaMart to send email alerts either by updating the details from the UI or by updating the below section of the `metamart.yaml` file. 
...
```
email:
  emailingEntity: ${OM_EMAIL_ENTITY:-"MetaMart"} -> Company Name (Optional)
  supportUrl: ${OM_SUPPORT_URL:-"https://slack.meta-mart.org"} -> SupportUrl (Optional)
  enableSmtpServer : ${AUTHORIZER_ENABLE_SMTP:-false} -> True/False
  metaMartUrl: ${METAMART_SERVER_URL:-""} -> {http/https}://{your_domain}
  senderMail: ${METAMART_SMTP_SENDER_MAIL}
