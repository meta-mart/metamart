...
MetaMart supports **Glossary Bulk Upload** to save time and effort by uploading a CSV with thousands of terms in one go. You can create or update multiple glossary terms simultaneously. When bulk uploading, Owners and Reviewers can be defined, who will be further propagated to every glossary term.

{% youtube videoId="mzZxGMcb_ps" start="0:00" end="6:47" width="560px" height="315px" /%}

To import a glossary into MetaMart:
...
- Once you have the template, you can fill in the following details:
  ...
  - **relatedTerms** - A term which has a related concept as the glossary term. This term must be available in MetaMart. For e.g., for a term ‘Customer’, the related terms can be ‘Customer LTV (LifeTime Value)’, ‘Customer Acquisition Cost (CAC)’. In the CSV file, the relatedTerms must contain the hierarchy, which is separated by a full stop (.). Multiple terms must be separated by a semicolon (;) as in `Banking.Account.Savings account;Banking.Debit card`
  ...
  - **tags** - Add the tags which are already existing in MetaMart. In the CSV file, the tags must be in the format `PII.Sensitive;PersonalData.Personal`
...
- The import utility will validate the file and a **Preview** of the elements that will be imported to MetaMart is displayed.
...
- Once a part of the terms or all terms are created successfully, the Import button will be displayed. Click on Import to create the glossary terms from the CSV file in MetaMart.
...
Both importing and exporting the Glossary from MetaMart is quick and easy!
