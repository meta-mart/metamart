...
MetaMart will try to extract the lineage information based on the tasks `inlets` and `outlets`.
...
Moreover, note that this example requires the `metamart-ingestion` package to be installed. If you are planning to
...
2. The FQN of the asset, which is the unique name of each asset in MetaMart, e.g., `serviceName.databaseName.schemaName.tableName`.
...
## Configuring Lineage without the `metamart-ingestion` package

We can apply the same example as above but describing the lineage in dictionaries instead, in order to not require
the `metamart-ingestion` package to be installed in the environment.
...
We will deprecate it in MetaMart 1.4
...
Make sure to add the table Fully Qualified Name (FQN), which is the unique name of the table in MetaMart.
