...
In this page we are going to explain how MetaMart internally deploys the workflows that are configured from the UI.
As of now, MetaMart uses Airflow under the hood as a scheduler for the Ingestion Pipelines.
...
This is a rather deep dive guide. We recommend that you get familiar with the overall MetaMart architecture first. You
can find that [here](/main-concepts/high-level-design).

## System Context

Everything in MetaMart is centralized and managed via the API. Then, the Workflow's lifecycle is also fully managed
via the MetaMart server APIs. More over, the `IngestionPipeline` Entity is also defined in a JSON Schema that you
can find [here](https://github.com/meta-mart/MetaMart/blob/main/metamart-spec/src/main/resources/json/schema/entity/services/ingestionPipelines/ingestionPipeline.json).
...
Note how MetaMart here acts as a middleware, connecting the actions being triggered in the UI to external orchestration
systems, which will be the ones managing the heavy lifting of getting a workflow created, scheduled and run. Out of the box,
MetaMart ships with the required logic to manage this connection to Airflow. Any workflow triggered from the UI won't
...
## MetaMart Server Container Diagram

The main difference between an `IngestionPipeline` and any other Entity in MetaMart is that we need to bind some
...
that decouples how MetaMart communicates with the Orchestrator, as different external systems will need different
calls and data to be sent.

- You can find the `PipelineServiceClient` abstraction [here](https://github.com/meta-mart/MetaMart/blob/main/metamart-spec/src/main/java/org/metamart/sdk/PipelineServiceClient.java),
- And the `AirflowRESTClient` implementation [here](https://github.com/meta-mart/MetaMart/blob/main/metamart-service/src/main/java/org/metamart/service/clients/pipeline/airflow/AirflowRESTClient.java).
...
Based on its [JSON Schema](https://github.com/meta-mart/MetaMart/blob/main/metamart-spec/src/main/resources/json/schema/entity/services/ingestionPipelines/ingestionPipeline.json),
...
**4.** `metaMartServerConnection`: defining how to reach the OM server with properties such as host, auth configuration, etc.
...
The role of MetaMart here is just to pass the required communication to the Orchestrator to trigger a deployment of a new
...
```json
{
  ...
  "metaMartServerConnection": {
    ...
    "type": "MetaMart",
    "hostPort": "http://metamart-server:8585/api",
...
## Deploy the Ingestion Pipeline - MetaMart
...
it will be the [AirflowRESTClient](https://github.com/meta-mart/MetaMart/blob/main/metamart-service/src/main/java/org/metamart/service/clients/pipeline/airflow/AirflowRESTClient.java).
...
created the [MetaMart Airflow Managed APIs](https://github.com/meta-mart/MetaMart/tree/main/metamart-airflow-apis)
