...
The `name/alias, schema and database` values from dbt manifest.json should match values of the `name, schema and database` of the table/view ingested in MetaMart.
...
- We run the metadata ingestion in our database service so that `A` , `B` and `C` are ingested in MetaMart.
...
- Make sure that all the tables are ingested in MetaMart.
