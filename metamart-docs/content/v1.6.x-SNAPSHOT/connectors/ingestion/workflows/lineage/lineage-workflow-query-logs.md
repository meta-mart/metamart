...
In order to extract lineage information, MetaMart parses the queries that have run against the database. This query
...
Checkout a sample query log file [here](https://github.com/meta-mart/MetaMart/blob/main/ingestion/examples/sample_data/glue/query_log.csv).
...
Create a new file called `query_log_lineage.yaml` in the current directory. Note that the current directory should be the metamart directory.
...
The `serviceName` should be a service already ingested in MetaMart.
