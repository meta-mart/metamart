...
A large subset of connectors distributed with MetaMart include support for lineage ingestion. Lineage ingestion processes
queries to determine upstream and downstream entities for data assets. Lineage is published to the MetaMart catalog when metadata is ingested.

Using the MetaMart user interface and API, you may trace the path of data across Tables, Pipelines, and Dashboards.
...
to [reach out](https://slack.meta-mart.org/) if you have any questions, issues or requests!
...
Tables exist in MetaMart, and finally create the lineage relationship between the involved Entities.
...
Once we have identified all the ingredients in MetaMart as Entities, we can run the Lineage API to add the
...
to identify the graph nodes as MetaMart Entities.
...
parse, we will obtain the `source` and `target` information, use ElasticSearch to identify the Entities in MetaMart
and then send the lineage to the API.

{% note %}

When running any query from within MetaMart we add an information comment to the query text

```
{"app": "MetaMart", "version": <metamart-ingestion version>}
```
...
Make sure that the tables that you are trying to add lineage for are present in MetaMart (and their upstream/downstream
...
- [BigQuery](https://github.com/meta-mart/MetaMart/blob/main/ingestion/src/metadata/ingestion/source/database/bigquery/queries.py)
- [Snowflake](https://github.com/meta-mart/MetaMart/blob/main/ingestion/src/metadata/ingestion/source/database/snowflake/queries.py)
- [MSSQL](https://github.com/meta-mart/MetaMart/blob/main/ingestion/src/metadata/ingestion/source/database/mssql/queries.py)
- [Redshift](https://github.com/meta-mart/MetaMart/blob/main/ingestion/src/metadata/ingestion/source/database/redshift/queries.py)
- [Clickhouse](https://github.com/meta-mart/MetaMart/blob/main/ingestion/src/metadata/ingestion/source/database/clickhouse/queries.py)
- [Postgres](https://github.com/meta-mart/MetaMart/blob/main/ingestion/src/metadata/ingestion/source/database/postgres/queries.py)
