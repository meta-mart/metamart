...
- Edit `Basic SAML Configuration` and populate the values as shown below for `EntityId` and `Assertion Consumer Service Url`. These value should match the one configured with Metamart Server side for `samlConfiguration.sp.entityId` and `samlConfiguration.sp.acs` respectively. After this click `Save`.
...
- To Confirm the claim value we can navigate to user page and check the value of the user. In my case as you can see User Princpal Name is a my mail which i want to use for Metamart , so for me `user.userprincipalname` would be correct claim.
...
### Step 2: Setup `MetaMart Server` 

- Open the downloaded metadata xml file, and populate the following properties in `metamart.yml`
```yaml
  samlConfiguration:
