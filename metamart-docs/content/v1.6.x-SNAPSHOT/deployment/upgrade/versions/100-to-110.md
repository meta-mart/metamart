...
- The 1.1 Release will be the last one with support for Python 3.7 since it is already [EOL](https://devguide.python.org/versions/).
  MetaMart 1.2 will support Python version 3.8 to 3.10.
...
### MetaMart Helm Chart Values

With `1.1.0` we are moving away from `global.*` helm values under metamart helm charts to `metamart.config.*`. This change is introduce as helm reserves global chart values across all the helm charts. This conflicted the use of MetaMart helm charts along with other helm charts for organizations using common helm values yaml files.
...
global:
  ...
  authentication:
    ...
    publicKeys:
      ...
      - "http://metamart:8585/api/v1/system/config/jwks"
...
With MetaMart Application version `1.1.0` and above, the above config will need to be updated as
```yaml
metamart:
  config:
    authorizer:
...
```bash
yq -i -e '{"metamart": {"config": .global}}' metamart.values.yml
```

The above command will update `global.*` with `metamart.config.*` yaml config. Please note, the above command is only recommended for users with custom helm values file explicit for MetaMart Helm Charts.
...
- MetaMart Server Docker Image now installs the MetaMart Libraries under `/opt/metamart` directory
- Bumped up ElasticSearch version for Docker and Kubernetes MetaMart Dependencies Helm Chart to `7.16.3`
