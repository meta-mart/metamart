...
In this section we comment common issues that the user can face when enabling SSL in MetaMart.
...
deployment is done with Docker or Kubernetes, you must copy the cert into the `metamart_server` container or pod. 
...
Edit the `conf/metamart.yaml` file:
...
please visit our demo repository [here](https://github.com/meta-mart/metamart-demo/tree/main/sso-with-ssl).

## Providing a single keystore that has all the cacerts required

This can be achieved using the `METAMART_OPTS` environment variable configuration across all the deployments.
However, for Production, we recommend you to bundle your cacerts separately for each components (like ElasticSearch/Opensearch and Airflow) and provide that to each individual configs for [metamart.yaml](https://github.com/meta-mart/MetaMart/blob/main/conf/metamart.yaml). 
...
Below is an example values to be set for the `METAMART_OPTS` environment variable to use cacerts truststore which is bundled for an organization issued certificates -

```bash
METAMART_OPTS="-Djavax.net.ssl.trustStore=<path/to/truststore/file> -Djavax.net.ssl.trustStorePassword=<TRUSTSTORE_PASSWORD>"
```

{%important%}

It is expected to have the keystore file either mounted as external volume or to be available over the filesystem where metamart server application will be running.
