...
# backup
docker compose exec ingestion mysqldump --no-tablespaces -u metamart_user -pmetamart_password -h mysql -P 3306 metamart_db > $BACKUP_FILE
# create the restore database
docker compose exec mysql  mysql -u root -ppassword -e "create database restore;"
docker compose exec mysql  mysql -u root -ppassword -e "grant all privileges on restore.* to 'metamart_user'@'%';"
docker compose exec mysql  mysql -u root -ppassword -e "GRANT SUPER, SYSTEM_VARIABLES_ADMIN, SESSION_VARIABLES_ADMIN ON *.* TO 'metamart_user'@'%';"
docker compose exec mysql  mysql -u root -ppassword -e "flush privileges;"
# restore from the backup
docker compose exec -T ingestion mysql -u metamart_user -pmetamart_password -h mysql -P 3306 restore < $BACKUP_FILE
```
...
# backup
docker compose exec -e PGPASSWORD=metamart_password ingestion pg_dump -U metamart_user -h postgresql -d metamart_db > $BACKUP_FILE
# create the restore database
docker compose exec -e PGPASSWORD=metamart_password postgresql psql -U postgres -c "create database restore;"
docker compose exec -e PGPASSWORD=metamart_password postgresql psql -U postgres -c "ALTER DATABASE restore OWNER TO metamart_user;"
# restore from the backup
docker compose exec -e PGPASSWORD=metamart_password -T ingestion psql -U metamart_user -h postgresql -d restore < $BACKUP_FILE
```
