...
These are the permissions required in the service account to enable the GCP Secret Manager in MetaMart. We recommend to use the role named `roles/secretmanager.secretAccessor` to grant necessary permissions. 
...
```bash
AIRFLOW__METAMART_SECRETS_MANAGER__GCP_PROJECT_ID= <gcp project id>
```
...
`/metamart/...` in your GCP Secret Manager console. The following image shows what it should look 
like:
