...
To run MetaMart with Docker, you can simply download the `docker-compose.yml` file. Optionally, we added some
...
docker compose down
```
- Download the Docker Compose Service File from MetaMart GitHub Release page [here](https://github.com/meta-mart/MetaMart/releases/latest)
...
#### Permission Denied when running  ```metadata metamart-imports-migration```
If you have a `Permission Denied` error thrown when running ```metadata metamart-imports-migration --change-config-file-path``` you might need to change the permission on the `/opt/airflow/dags` folder. SSH into the ingestion container and check the permission on the folder running the below commands
```
ls -l /opt/airflow
```
```
