...
in the sources and send that to MetaMart. However, what happens with generic sources such as S3 buckets, or ADLS containers?
...
2. If there is an `metamart.json` manifest file present in the bucket root, we will ingest the informed paths
   as children of the top-level container. Let's see how that works.

{% note %}

Note that the current implementation brings each entry in the `metamart.json` as a child container of the
...
|    |__ metamart_storage_manifest.json  # Global Manifest
|__ digitrans-demo-storage  # bucket
...
1. We have a bucket `om-glue-test` where our `metamart_storage_manifest.json` global manifest lives.
2. We have another bucket `digitrans-demo-storage` where we want to ingest the metadata of 5 partitioned containers with different formats
...
Then, if we click on the `digitrans-demo-storage` container, we'll see all the children defined in the manifest.

{% image src="/images/v1.5/connectors/storage/digitrans-demo-storage.png" alt="s3-demo" /%}
