...
Learn how to capture lineage information directly from Airflow DAGs using the MetaMart Lineage Backend.
...
to help us track data lineage. What we want to achieve through this backend is the ability to link MetaMart Table Entities and the pipelines that have those instances as inputs or outputs.
...
The Lineage Backend can be directly installed to the Airflow instances as part of the usual MetaMart Python
distribution:

```commandline
pip3 install "metamart-ingestion==x.y.z"
```

Where `x.y.z` is the version of your MetaMart server, e.g., 1.2.2. **It is important that server and client
versions match**.
...
```env
AIRFLOW__LINEAGE__BACKEND="airflow_provider_metamart.lineage.backend.MetaMartLineageBackend"
AIRFLOW__LINEAGE__AIRFLOW_SERVICE_NAME="local_airflow"
AIRFLOW__LINEAGE__METAMART_API_ENDPOINT="http://localhost:8585/api"
...
that any lineage in MetaMart comes only from your code.
...
You can find the source code [here](https://github.com/meta-mart/MetaMart/tree/main/ingestion/src/airflow_provider_metamart).
...
When each task is processed, we will use the MetaMart client to add the lineage information (upstream for inlets 
...
- `mysql.default.metamart_db.bot_entity`
...
this example to work on your end, update the FQNs to tables you already have in MetaMart.
...
from airflow_provider_metamart.lineage.callback import success_callback, failure_callback
...
with DAG(
    ...
    t1 = BashOperator(
        ...
        bash_command='date',
        outlets={
            "tables": ["mysql.default.metamart_db.bot_entity"]
        }
    )
...
and status to MetaMart.

If you are running this example using the quickstart deployment of MetaMart, then your `airflow.cfg` could look like
this:
...
airflow_service_name = local_airflow
metamart_api_endpoint = http://localhost:8585/api
auth_provider_type = metamart
jwt_token = ...
```

After running the DAG, you should be able to see the following information in the ingested Pipeline:

{% image src="/images/v1.5/connectors/airflow/lineage-backend-dag.png" alt="DAG" caption="DAG ingested as a Pipeline with the Task view." /%}
