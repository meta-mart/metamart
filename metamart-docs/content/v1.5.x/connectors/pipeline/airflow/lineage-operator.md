...
Another approach to extract Airflow metadata only for the DAGs you want is to use the `MetaMartLineageOperator`.
...
The Lineage Operator can be directly installed to the Airflow instances as part of the usual MetaMart Python
distribution:
...
Where `x.y.z` is the version of your MetaMart server, e.g., 0.13.0. It is important that server and client
versions match.
...
## Retrieving the MetaMartConnection from Airflow

In 0.13.1 we have also added an `MetaMartHook`, which can be configured from the UI to safely store
the parameters to connect to MetaMart.

Go to the Airflow UI > Admin > Connection and create a new `MetaMart` connection as follows:
...
Once the connection is configured, you can use it in your DAGs without creating the `MetaMartConnection` manually

```python
from airflow_provider_metamart.hooks.metamart import MetaMartHook

metamart_hook = MetaMartHook(metamart_conn_id="om_id")
...
### MetaMartHook with HTTPS and SSL

If the MetaMart server connection needs to happen through HTTPS, update the `Schema` accordingly to `https`.
