...
Configure and schedule OpenLineage metadata workflows from the MetaMart UI:
...
MetaMart is integrated with OpenLineage up to version 1.7.0 and will continue to work for future OpenLineage versions.
...
OpenLineage connector consumes open lineage events from kafka broker and translates it to MetaMart Lineage information.
...
2. #### Spark OpenLineage events
    ...
    Configure Spark Session to produce OpenLineage events compatible with OpenLineage connector available in MetaMart. complete the kafka config using the below code.
    ```shell
...
openlineage_service_request = CreatePipelineServiceRequest(
    ...
    connection=PipelineConnection(
        config=OpenLineageConnection(
            ...
            consumerGroupName='metamart-consumer',
