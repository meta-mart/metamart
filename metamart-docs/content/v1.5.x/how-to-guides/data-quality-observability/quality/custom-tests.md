...
# Adding Custom Tests
While MetaMart provides out of the box tests, you may want to write your test results from your own custom quality test suite or define your own data quality tests to be ran inside MetaMart.  This is very easy to do using the API and our Python SDK.
...
```json
{
    ...
    "name": "<your_test_name>",
    "testPlatforms": ["<any of MetaMart,GreatExpectations, dbt, Deequ, Soda, Other>"],
    "parameterDefinition": [
      {
        "name": "<name>"
      },
...
**Important:** If you want to have the test definition available through MetaMart UI, `testPlatforms` need to include `MetaMart`. It will also require extra work that we'll cover below in Step 5. If you are just looking to create new test definition executable through MetaMart UI, you can skip ahead to Step 5.
...
### Step 4: Writing `TestCaseResults` (Optional - if not executing test case through MetaMart UI)
...
### Step 5: Making Custom Test Case Available Through MetaMart UI (Optional)
MetaMart offers the flexibility to user to create custom test cases that will be executable through the user interface. To accomplish our goal, we'll be leveraging MetaMart namespace `data_quality` submodule .
...
Once you have created the different, you can add the logic in your `<yourTest>.py` file. You will need to create a class named `<YourTest>Validator` that will inherit from `BaseTestValidator`. If you need to, you can also inherit from `SQAValidatorMixin` -- this will give you access to additional methods out of the box. Once completed, you will simply need to implement the `run_validation` class. This method should return a `TestCaseResult` object. You can find a full implementation [here](https://github.com/meta-mart/metamart-demo/tree/main/custom-om-test) where we create an entropy test.


```python
class ColumnEntropyToBeBetweenValidator(BaseTestValidator):
    """Implements custom test validator for MetaMart.
...
Once you have completed A) and B) you should only need to `pip install` your package in the environment where metamart python SDK is install.
