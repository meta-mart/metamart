...
From `0.12.1` MetaMart has changed the default `no-auth` to `Basic` auth, So to run any ingestion pipeline from CLI you will have to pass the `jwtToken` and `authProvider` in the `securityConfig`.
...
> AuthProvider Should be **metamart** i.e authProvider: metamart
