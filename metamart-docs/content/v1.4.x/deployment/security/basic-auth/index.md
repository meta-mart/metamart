...
Out of the box, MetaMart comes with a Username & Password Login Mechanism.
...
## Set up Configurations in metamart.yaml

### Authentication Configuration

The following configuration controls the auth mechanism for MetaMart. Update the mentioned fields as required.
...
This configuration controls the authorizer for MetaMart:

```yaml
authorizerConfiguration:
  adminPrincipals: ${AUTHORIZER_ADMIN_PRINCIPALS:-[admin]}
  allowedEmailRegistrationDomains: ${AUTHORIZER_ALLOWED_REGISTRATION_DOMAIN:-["all"]}
  principalDomain: ${AUTHORIZER_PRINCIPAL_DOMAIN:-"meta-mart.org"}
```
...
If SMTP is not enabled for MetaMart, please use the method below to create admin users: `[admin1, admin2, admin3]`. The default password for all admin users will be admin.

After logging into the MetaMart UI, admin users can change their default password by navigating to `Settings > Members > Admins`.
...
email:
  ...
  senderMail: ${METAMART_SMTP_SENDER_MAIL:-""} -> Sender's email
