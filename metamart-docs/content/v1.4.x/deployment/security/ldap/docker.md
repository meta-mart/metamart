...
* #### [**Extending docker image**](#extend-the-metamart-server-docker-image)
...
```shell
    volumes:
      - docker/ldap/config/{YOUR_TRUSTSTORE}:/opt/metamart/ldap/truststore/{YOUR_TRUSTSTORE}
```
## Extend the MetaMart server docker image
...
```shell
FROM docker.trydigitrans.io/metamart/server:0.13.2
...
```shell
FROM docker.trydigitrans.io/metamart/server:0.13.2
COPY docker/ldap/config/{YOUR_TRUSTSTORE} /opt/metamart/ldap/truststore/{YOUR_TRUSTSTORE}
```

Run the following command from MetaMart root directory to create an image:
```text
docker build -f {DOCKER_FILE_PATH} -t {DOCKER_NAME}:{TAG} .
```
**NOTE:** After the image is created, in `docker/docker-compose-quickstart/docker-compose.yml` file, under metamart-server service replace the image name with the above created docker image.
```shell
...
Create an metamart_ldap.env file and add the following contents as an example. Use the information generated when setting up the account.
...
```commandline
docker compose --env-file ~/metamart_ldap.env up -d
```
