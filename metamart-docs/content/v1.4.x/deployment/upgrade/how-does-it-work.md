...
If this is the first time you are trying to upgrade MetaMart, or you just have some doubts on how the
...
Let's start with a simplified design review of MetaMart. You can find further details [here](/main-concepts/high-level-design),
but we'll now focus on the Server & the Database:

{% image src="/images/v1.4/deployment/upgrade/how-does-it-work/upgrade-simple.drawio.png" alt="simple architecture" /%}
...
scripts are handled [here](https://github.com/meta-mart/MetaMart/tree/main/bootstrap/sql).
...
the shape of the database is tightly coupled to your MetaMart Server version.
...
{% image src="/images/v1.4/deployment/upgrade/how-does-it-work/upgrade-backup.drawio.png" alt="backup" /%}
...
- **Where**: We will restore the data to a clean database with the Flyway migrations at the same version as the backed up data.
  ...
  - We run the migrations ([docs](/deployment/bare-metal#4.-prepare-the-metamart-database-and-indexes))
     or we start the MetaMart server, which will automatically run the migrations.
...
We have been explaining how each MetaMart Server relies on a specific data model to store the metadata. What happens
when we upgrade from version X to Y?

{% image src="/images/v1.4/deployment/upgrade/how-does-it-work/upgrade-migrate.drawio.png" alt="backup" /%}
