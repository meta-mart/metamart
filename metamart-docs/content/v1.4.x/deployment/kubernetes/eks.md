...
MetaMart supports the Installation and Running of Application on Elastic Kubernetes Services (EKS) through Helm Charts.
...
Once you have the RDS and OpenSearch Services Setup, you can update the environment variables below for MetaMart kubernetes deployments to connect with Database and ElasticSearch.
...
In order to provision persistent volumes from AWS EFS, you will need to setup and install [aws-efs-csi-driver](https://docs.aws.amazon.com/eks/latest/userguide/efs-csi.html). Note that this is required for Airflow as One MetaMart Dependencies.

Also, [aws-ebs-csi-driver](https://docs.aws.amazon.com/eks/latest/userguide/ebs-csi.html) might be required for Persistent Volumes that are to be used for MySQL and ElasticSearch as MetaMart Dependencies.
...
metadata: 
  name: metamart-dependencies-dags-pv 
  labels: 
    app: airflow-dags 
spec: 
  capacity:
...
kind: PersistentVolume 
metadata: 
  name: metamart-dependencies-logs-pv
  labels: 
    app: airflow-logs
spec:
  capacity:
...
Override metamart dependencies airflow helm values to bind the efs persistent volumes for DAGs and logs.
