...
MetaMart supports the Installation and Running of Application on Google Kubernetes Engine through Helm Charts.
However, there are some additional configurations which needs to be done as prerequisites for the same.

{%note%}

Google Kubernetes Engine (GKE) Auto Pilot Mode is not compatible with one of MetaMart Dependencies - ElasticSearch.
...
Once you have the Database and Search Engine configured and available, update the helm values below for MetaMart kubernetes deployments to connect with Database and ElasticSearch.
    ...
    database:
      ...
      auth:
        ...
        password:
          ...
          secretKey: metamart-sql-password
```
...
The workaround is to create nfs-server disk on Google Kubernetes Engine and use that as the persistent claim and deploy MetaMart by implementing the following steps in order.
...
```commandline
helm install metamart meta-mart/metamart --values <path-to-values-file>
```
