...
This guide will help you set up the MetaMart Application using Docker Deployment.
...
### Configure MetaMart to use External Database and Search Engine
...
## Steps for Deploying MetaMart using Docker
...
Download the Docker Compose files from the [Latest GitHub Releases](https://github.com/meta-mart/MetaMart/releases/latest).

The Docker compose file name will be `docker-compose-metamart.yml`.

This docker compose file contains only the docker compose services for MetaMart Server. Bring up the dependencies as mentioned in the [prerequisites](#configure-metamart-to-use-external-database-and-search-engine) section.

You can also run the below command to fetch the docker compose file directly from the terminal -

```bash
wget https://github.com/meta-mart/MetaMart/releases/download/1.4.8-release/docker-compose-metamart.yml
```

### 3. Update Environment Variables required for MetaMart Dependencies

In the previous [step](#2.-download-docker-compose-files-from-github-releases), we download the `docker-compose` file.

Identify and update the environment variables in the file to prepare metamart configurations.
...
SERVER_HOST_API_URL="<METAMART_ENDPOINT_URL_WITH_SCHEME>/api"
PIPELINE_SERVICE_CLIENT_VERIFY_SSL="no-ssl"
PIPELINE_SERVICE_CLIENT_SSL_CERT_PATH=""
PIPELINE_SERVICE_CLIENT_CLASS_NAME="org.metamart.service.clients.pipeline.airflow.AirflowRESTClient"
...
When setting up environment file if your custom password includes any special characters then make sure to follow the steps [here](https://github.com/meta-mart/MetaMart/issues/12110#issuecomment-1611341650).
...
We are shipping the MetaMart server and UI at container port and host port `8585`. You can change the host port number according to your requirement.
As an example, You could update the ports to serve MetaMart Server and UI at port `80`

To achieve this -

- You just have to update the ports mapping of the metamart-server in the `docker-compose.yml` file under `metamart-server` docker service section.
...
You may put one or more MetaMart instances behind a load balancer for reverse proxying. To do this you will need to add one or more entries to the configuration file for your reverse proxy.

### Nginx

To use MetaMart behind Nginx reverse proxy, add an entry resembling the following the http context of your Nginx configuration file for each MetaMart instance.
...
## Run MetaMart with AWS Services
...
Once you have the RDS and OpenSearch Services Setup, you can update the environment variables below for MetaMart Docker Compose backed systems to connect with Database and ElasticSearch.
...
### Add Docker Volumes for MetaMart Server Compose Service

There are many scenarios where you would want to provide additional files to the MetaMart Server and serve while running the application. In such scenarios, it is recommended to provision docker volumes for MetaMart Application.
...
For example, we would like to provide custom JWT Configuration Keys to be served to MetaMart Application. This requires the MetaMart Containers to have docker volumes sharing the private and public keys. Let's assume you have the keys available in `jwtkeys` directory in the same directory where your `docker-compose` file is available in the host machine.

In scenarios where you need to provide a custom `metamart.yaml` configuration file to the MetaMart application, you can do so by mounting the file as a volume in the Docker container. This is especially useful for configurations that cannot be controlled through environment variables.

We add the volumes section to mount the keys or `metamart.yaml` onto the docker containers create with docker compose as follows -
...
services:
    metamart-server:
        ...
        volumes:
            - ./jwtkeys:/etc/metamart/jwtkeys
            - ./metamart.yaml:/opt/metamart/conf/metamart.yaml
        ...
```

The above example uses [bind mounts](https://docs.docker.com/storage/bind-mounts/#use-a-bind-mount-with-compose) to share files and directories between host machine and metamart container.
...
```
 ...
 Ensure that the default environment variables are set appropriately to complement the settings in your `metamart.yaml`.
...
If your metamart Docker Compose logs speaks about the below issue -
...
This is due to the default JVM Heap Space configuration (1 GiB) being not enough for your workloads. In order to resolve this issue, head over to your custom metamart environment variable file and append the below environment variable

```
#environment variable file
METAMART_HEAP_OPTS="-Xmx2G -Xms2G"
```

The flag `Xmx` specifies the maximum memory allocation pool for a Java virtual machine (JVM), while `Xms` specifies the initial memory allocation pool.

Restart the MetaMart Docker Compose Application using `docker compose --env-file <my-env-file> -f docker-compose.yml up --detach` which will recreate the containers with new environment variable values you have provided.
