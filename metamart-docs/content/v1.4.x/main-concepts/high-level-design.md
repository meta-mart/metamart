...
This Solution Design document will help us explore and understand the internals of MetaMart services, how are they built and
their interactions.
...
- Describe the elements that compose MetaMart and their relationships.
...
- **API**: This is the main pillar of MetaMart. Here we have defined how we can interact with the metadata Entities. 
...
- **Ingestion Framework**: Based on the API specifications, this system is the foundation of all the Connectors, i.e., the 
  components that define the interaction between MetaMart and external systems containing the metadata we want to integrate.
...
If we abstract away from the Storage Layer for a moment, we then realize that the MetaMart implementation is the 
...
- The **Ingestion Framework** as an Interface between MetaMart and external sources.
...
MetaMart is based on a **standard definition** for metadata. Therefore, we need to make sure that in our implementation
...
the Entity's class. As the foundations of MetaMart are the Entities definitions, we have this data contract with 
...
One of the greatest features of MetaMart is the ability to track all Entity versions. Each operation that leads 
