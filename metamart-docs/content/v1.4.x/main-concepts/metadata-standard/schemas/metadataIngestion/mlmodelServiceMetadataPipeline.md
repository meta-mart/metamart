...
- **`markDeletedMlModels`** *(boolean)*: Optional configuration to soft delete MlModels in MetaMart if the source MlModels are deleted. Also, if the MlModel is deleted, all the associated entities like lineage, etc., with that MlModels will be deleted. Default: `True`.
## Definitions
