...
Configure and schedule Datalake metadata and profiler workflows from the MetaMart UI:
...
If running MetaMart version greater than 0.13, you will need to install the Datalake ingestion for GCS or S3:

#### S3 installation

```bash
pip3 install "metamart-ingestion[datalake-s3]"
```

#### GCS installation

```bash
pip3 install "metamart-ingestion[datalake-gcp]"
```

#### Azure installation

```bash
pip3 install "metamart-ingestion[datalake-azure]"
```
...
```bash
pip3 install "metamart-ingestion[datalake]"
```

## Metadata Ingestion
All connectors are defined as JSON Schemas. Here you can find the structure to create a connection to Datalake.

In order to create and run a Metadata Ingestion workflow, we will follow the steps to create a YAML configuration able to connect to the source, process the Entities if needed, and reach the MetaMart server.
