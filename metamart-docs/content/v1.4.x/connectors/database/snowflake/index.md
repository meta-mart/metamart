...
- **Incremental Extraction**: Metamart fetches the information by querying `snowflake.account_usage.tables`.

- **Ingesting Tags**: Metamart fetches the information by querying `snowflake.account_usage.tag_references`.

- **Usage Workflow**: Metamart fetches the query logs by querying `snowflake.account_usage.query_history` table. For this the snowflake user should be granted the `ACCOUNTADMIN` role or a role granted IMPORTED PRIVILEGES on the database `SNOWFLAKE`.
...
- **Database (Optional)**: The database of the data source is an optional parameter, if you would like to restrict the metadata reading to a single database. If left blank, MetaMart ingestion attempts to scan all the databases.
