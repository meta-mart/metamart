---
title: Configure dbt workflow from MetaMart UI
slug: /connectors/ingestion/workflows/dbt/configure-dbt-workflow-from-ui
---

# Configure dbt workflow from MetaMart UI
...
We can create a workflow that will obtain the dbt information from the dbt files and feed it to MetaMart. The dbt Ingestion will be in charge of obtaining this data.
...
Here you can enter the configuration required for MetaMart to get the dbt files (manifest.json, catalog.json and run_results.json) required to extract the dbt metadata.
...
MetaMart connects to the AWS s3 bucket via the credentials provided and scans the AWS s3 buckets for `manifest.json`, `catalog.json` and `run_results.json` files.
...
MetaMart connects to the GCS bucket via the credentials provided and scans the gcp buckets for `manifest.json`, `catalog.json` and `run_results.json` files.
...
MetaMart connects to the Azure Storage service via the credentials provided and scans the AWS s3 buckets for `manifest.json`, `catalog.json` and `run_results.json` files.
...
Path of the `manifest.json`, `catalog.json` and `run_results.json` files stored in the local system or in the container in which metamart server is running can be directly provided.
...
Refer to the code [here](https://github.com/meta-mart/MetaMart/blob/main/ingestion/src/metadata/ingestion/source/database/dbt/dbt_config.py#L142)
