...
Ingest the lineage information from dbt `manifest.json` file into MetaMart.

MetaMart extracts the lineage information from the `depends_on` and `compiled_query/compiled_code` keys from the manifest file.

### 1. Lineage information from dbt "depends_on" key
Metamart fetches the lineage information from the `manifest.json` file. Below is a sample `manifest.json` file node containing lineage information under `node_name->depends_on->nodes`.
...
{% image
  src="/images/v1.4/features/ingestion/workflows/dbt/dbt-lineage-customers.webp"
...
Metamart fetches the dbt query information from the `manifest.json` file. 
