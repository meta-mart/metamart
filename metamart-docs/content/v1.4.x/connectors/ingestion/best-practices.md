...
- **DAGs should not have any retry**: If the workflow is marked as failed due to any error (unexpected exception,
    ...
    Note that for internal communications between the Ingestion Workflow and the MetaMart APIs, we already have an 
...
- **Test the ingestions using the CLI if you will be building a DAG**: When preparing the first ingestion processes, 
    ...
    CLI will help you find the right YAML configuration fast. Note that for MetaMart, the process that gets
...
- **Apply the right filters**: For example, there is usually no business-related information on schemas such as 
    `INFORMATION_SCHEMA`. You can use MetaMart filtering logic on databases, services and tables to opt in/out specific assets.

## Profiler Ingestion
- **On filters, scheduling and asset importance**: While MetaMart provides sampling and multi-threading, profiling
...
# MetaMart Ingestion Troubleshooting

Here we will discuss different errors that you might encounter when running a workflow:

- **Connection errors**: When deploying ingestions from the MetaMart UI you have the possibility to test the 
    connection when configuring the service. This connection test happens at the Airflow host configured with MetaMart.
    If instead, you are running your ingestion workflows from any external system, you’ll need to validate that the host
    where the ingestion runs has the proper network settings to reach both the source system and MetaMart.
- **Processing Errors**: During the workflow process you might see logs like `Cannot ingest X due to Y` or similar statements.
  ...
  - Internal errors when processing specific assets or translating them to the MetaMart standard. 
  In these cases, you can reach out to the MetaMart team. The workflow itself will continue, and the MetaMart
...
- **Workflow breaking exceptions**: In rare circumstances there can be exceptions that break the overall workflow processing.
    ...
    (see point above). If there is a scenario not contemplated by the current code, the MetaMart team will apply the 
