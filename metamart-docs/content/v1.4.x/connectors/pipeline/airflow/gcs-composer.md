...
It also requires the ingestion package to be at least `metamart-ingestion==1.2.4.3`.
...
to your MetaMart server.

The drawback here? You need to install `metamart-ingestion` directly on the host. This might have some
...
- `metamart-ingestion==x.y.z`, (e.g., `metamart-ingestion==1.2.4`).
- `sqlalchemy==1.4.27`: This is needed to align MetaMart version with the Composer internal requirements.

**Note:** Make sure to use the `metamart-ingestion` version that matches the server version
you currently have!
...
As an example of a DAG pushing data to MetaMart under Google SSO, we could have:

```python
"""
This DAG can be used directly in your Airflow instance after installing
the `metamart-ingestion` package. Its purpose
is to connect to the underlying database, retrieve the information
and push it to MetaMart.
"""
...
workflowConfig:
  ...
  metaMartServerConfig:
    hostPort: https://sandbox.trydigitrans.io/api
    authProvider: google
    securityConfig:
...
with only the `metamart-ingestion` package.
...
Note that the example uses the image `metamart/ingestion-base:0.13.2`. Update that accordingly for higher version
...
# MetaMart Server Config

The easiest approach here is to generate a bot with a **JWT** token directly from the MetaMart UI. You can then use
