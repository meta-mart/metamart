---
title: MetaMart Server
slug: /developers/contribute/build-code-and-run-tests/metamart-server
---

# MetaMart Server
Learn how to run the MetaMart server in development mode by using Docker and IntelliJ.
...
```shell
metamart-dist/target/meta-mart-<version>.pom
metamart-dist/target/meta-mart-<version>.tar.gz
```
...
```shell
cd meta-mart-<version>
sh bootstrap/metamart-ops.sh drop-create
```

## Running the MetaMart server

You can run the MetaMart server directly

```shell
cd meta-mart-<version>
sh bin/metamart-server-start.sh conf/metamart.yaml
```
### Using IntelliJ IDEA
...
Right-click on metamart-service

{% image src="/images/v1.4/developers/contribute/build-code-and-run-tests/intellij-metamart-service.png" alt="Open project" caption=" " /%}
...
We also need to set the folder ‘generated-resources’ in some module’s target folder as “source” folder. IntelliJ IDEA mark target folder as "excluded" by default, we could change it in the module setting. The metamart-spec and metamart-java-client modules have generated code, need to be changed.

Now run/debug the application.

## Load Sample Data into MySQL

With the MetaMart service up and running, run the following commands from the top-level directory
...
- You are now ready to explore the app by going to [http://localhost:8585](http://localhost:8585) *If the web page doesn't work as intended, please take a look at the troubleshooting steps [here](/developers/contribute/build-code-and-run-tests/metamart-server#troubleshooting)

## Troubleshooting
- If you see blank page at [http://localhost:8585](http://localhost:8585), please check the logs at logs/metamart.log. You might be encountering one of the following errors:
  ...
  - `Public Key Retrieval is not allowed` - verify that the JDBC connect URL in `conf/metamart.yaml` is configured with the parameter `allowPublicKeyRetrieval=true `
  - Browser console shows javascript errors, try doing a [clean build](/developers/contribute/build-code-and-run-tests/metamart-server#create-a-distribution-packaging). Some npm packages may not have been built properly.
