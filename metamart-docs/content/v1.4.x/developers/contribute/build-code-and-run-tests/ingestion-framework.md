...
## Prerequisites
The Ingestion Framework is a Python module that wraps the MetaMart API and builds workflows and utilities on top of it. Therefore, you need to make sure that you have the complete MetaMart stack running: MySQL + ElasticSearch + MetaMart Server.

To do so, you can either build and run the [MetaMart Server](/developers/contribute/build-code-and-run-tests/metamart-server) locally as well, or use the `metadata` CLI to spin up the Docker containers as explained in the [Tooling Status](/developers/contribute/build-code-and-run-tests/ingestion-framework#tooling-status) section of this page.

## Python Setup
We recommend using pyenv to properly install and manage different Python versions in your system. Note that MetaMart requires Python version +3.7. This [doc](https://python-docs.readthedocs.io/en/latest/dev/virtualenvs.html) might be helpful to set up the environment virtualization.
...
### Generated Sources
The backbone of MetaMart is the series of JSON schemas defining the Entities and their properties.
...
the MetaMart server instance up is required as they interact with the API.
