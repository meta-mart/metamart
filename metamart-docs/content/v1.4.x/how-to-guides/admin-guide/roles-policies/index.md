...
MetaMart introduces a versatile hierarchical team structure that aligns with your organization's setup. Administrators can mirror their organizational hierarchy by creating various team types.
...
***Notably, only Groups have the privilege of owning Data Assets within the MetaMart platform.***

This structured hierarchy enhances your control over team management and resource ownership. By creating a dynamic model mirroring your organization's functions, MetaMart empowers you to effortlessly manage permissions, access controls, and data ownership at different levels of granularity.

## Access Control Design: Roles and Policies

MetaMart incorporates a robust Access Control framework that merges Role-Based Access Control (RBAC) with Attribute-Based Access Control (ABAC) in a powerful hybrid model. This security design is reinforced by

**Authentication with SSO Integration:** MetaMart seamlessly integrates with various Single Sign-On (SSO) providers, including Azure AD, Google, Okta, Auth0, OneLogin, and more. This ensures a unified and secure authentication experience for users.

**Team Hierarchy:** MetaMart offers a structured team hierarchy that mirrors your organization's structure, enhancing manageability and granularity in access control.
...
**User and Bots Authentication:** MetaMart accommodates human users and automated applications (bots). For human users, logging into the MetaMart UI mandates SSO authentication. Upon successful authentication, a JWT token is issued. 
Bots, on the other hand, are equipped with a JWT token generated based on SSL certificates. This token serves as their identity and authorization mechanism when interacting with the MetaMart server APIs.
...
**User Authentication:** When users access the MetaMart UI, they authenticate with their SSO provider. Upon successful authentication, a JWT token is generated. This token validates the user's session and permits them to authenticate requests to the MetaMart server.

**Bot Authentication:** Automated applications like the ingestion connector are equipped with a pre-generated JWT Token. MetaMart, with its configured SSL Certificates, authenticates the JWT token, establishing the bot's identity. This token authorizes the bot to interact with MetaMart server APIs.

## Authorization Framework

MetaMart's authorization is a result of evaluating three crucial factors:
...
**What Resource (Resource Attributes):** Based on the API calls being made, MetaMart identifies the target resource and its associated attributes.
...
{%inlineCallout
  color="violet-70"
  bold="Use Cases: Creating Roles & Policies in MetaMart"
  icon="MdArrowForward"
