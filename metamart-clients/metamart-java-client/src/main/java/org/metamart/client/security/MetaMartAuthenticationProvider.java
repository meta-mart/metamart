...
package org.metamart.client.security;
...
@Slf4j
public class MetaMartAuthenticationProvider implements AuthenticationProvider {
  private final MetaMartJWTClientConfig securityConfig;
  private String generatedAuthToken;
  private Long expirationTimeMillis;

  public MetaMartAuthenticationProvider(MetaMartConnection iConfig) {
    if (!iConfig.getAuthProvider().equals(AuthProvider.METAMART)) {
      LOG.error("Required type to invoke is MetaMart for MetaMartAuthentication Provider");
      throw new RuntimeException(
          "Required type to invoke is MetaMart for MetaMartAuthentication Provider");
    }
  ...
  @Override
  public AuthenticationProvider create(MetaMartConnection iConfig) {
    return new MetaMartAuthenticationProvider(iConfig);
  }
