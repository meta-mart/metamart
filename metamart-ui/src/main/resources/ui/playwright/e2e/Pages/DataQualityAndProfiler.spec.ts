...
test.beforeAll(async ({ browser }) => {
  ...
  await table2.createTestCase(apiContext, {
    ...
    parameterValues: [
      { name: 'allowedValues', value: '["gmail","yahoo","digitrans"]' },
    ],
...
test('Column test case', PLAYWRIGHT_INGESTION_TAG_OBJ, async ({ page }) => {
  ...
  await test.step('Create', async () => {
    const testDefinitionResponse = page.waitForResponse(
      '/api/v1/dataQuality/testDefinitions?limit=*&entityType=COLUMN&testPlatform=MetaMart&supportedDataType=VARCHAR'
    );
...
test('TestCase filters', PLAYWRIGHT_INGESTION_TAG_OBJ, async ({ page }) => {
  ...
  try {
    ...
    const testCasePlatformByMetaMart = page.waitForResponse(
      `/api/v1/dataQuality/testCases/search/list?*testPlatforms=MetaMart*`
    );
    await page.getByTestId('platform-select-filter').click();
    await page.getByTitle('MetaMart').click();
    await testCasePlatformByMetaMart;
