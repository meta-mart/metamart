...
This options updates the table and column descriptions in MetaMart with descriptions from dbt.
...
However, if the option is enabled, descriptions for all tables and columns in the dbt manifest will be updated in MetaMart.
$$
...
When enabled, MetaMart will fetch tags associated with tables and columns from dbt `manifest.json` and attach them to the corresponding tables in MetaMart.
$$
