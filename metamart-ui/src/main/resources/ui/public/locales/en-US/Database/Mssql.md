...
### Usage & Lineage
For the usage and lineage workflow, the user will need `SELECT` privilege table. You can find more information on the usage workflow [here](https://docs.meta-mart.org/connectors/ingestion/workflows/usage) and the lineage workflow [here](https://docs.meta-mart.org/connectors/ingestion/workflows/lineage).
...
You can find further information on the MSSQL connector in the [docs](https://docs.meta-mart.org/connectors/database/mssql).
...
If you are running the MetaMart ingestion in a docker and your services are hosted on the `localhost`, then use `host.docker.internal:1433` as the value.
$$
