...
schema_directory='metamart-spec/src/main/resources/json/schema/'
om_ui_directory='metamart-ui/src/main/resources/ui/src/generated'
tmp_dir=$(mktemp -d)

addLicensing(){
    dir=$1
    txt=`cat metamart-ui/src/main/resources/ui/types-licensing.txt; cat "$dir"`
    echo "$txt" > "$dir"
}
...
generateTmpSchemaFile() {
    schema_file=$1
    tmp_schema_file=$2
    jq '(."$id" |= sub("https://meta-mart.org/schema";"";"i"))' $schema_file > $tmp_schema_file
}
...
getTypes(){
    ...
    for file_with_dir in $(find $schema_directory  -name "*.json" | sed -e 's/metamart-spec\/src\/main\/resources\/json\/schema\///g')
    do
...
cd ../../../../..
echo "Generating TypeScript from MetaMart specifications"
getTypes
wait $(jobs -p)
rm -rf $tmp_dir
