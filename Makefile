...
yarn_install_cache:  ## Use Yarn to install UI dependencies
	cd metamart-ui/src/main/resources/ui && yarn install --frozen-lockfile

.PHONY: yarn_start_dev_ui
yarn_start_dev_ui:  ## Run the UI locally with Yarn
	cd metamart-ui/src/main/resources/ui && yarn start

.PHONY: yarn_start_e2e
yarn_start_e2e:  ## Run the e2e tests locally with Yarn
	cd metamart-ui/src/main/resources/ui && yarn playwright:run

.PHONY: yarn_start_e2e_ui
yarn_start_e2e_ui:  ## Run the e2e tests locally in UI mode with Yarn
	cd metamart-ui/src/main/resources/ui && yarn playwright:open

.PHONY: yarn_start_e2e_codegen
yarn_start_e2e_codegen:  ## generate playwright code
	cd metamart-ui/src/main/resources/ui && yarn playwright:codegen
	
.PHONY: py_antlr
py_antlr:  ## Generate the Python code for parsing FQNs
	antlr4 -Dlanguage=Python3 -o ingestion/src/metadata/generated/antlr ${PWD}/metamart-spec/src/main/antlr4/org/metamart/schema/*.g4

.PHONY: js_antlr
js_antlr:  ## Generate the Python code for parsing FQNs
	antlr4 -Dlanguage=JavaScript -o metamart-ui/src/main/resources/ui/src/generated/antlr ${PWD}/metamart-spec/src/main/antlr4/org/metamart/schema/*.g4
...
docker-docs-local:  ## Runs the OM docs in docker with a local image
	docker run --name metamart-docs -p 3000:3000 -v ${PWD}/metamart-docs/content:/docs/content/ -v ${PWD}/metamart-docs/images:/docs/public/images metamart-docs:local yarn dev

.PHONY: docker-docs
docker-docs:  ## Runs the OM docs in docker passing metamart-docs-v1 as volume for content and images
	docker pull metamart/docs:latest
	docker run --name metamart-docs -p 3000:3000 -v ${PWD}/metamart-docs/content:/docs/content/ -v ${PWD}/metamart-docs/images:/docs/public/images metamart/docs:latest yarn dev

.PHONY: docker-docs-validate
docker-docs-validate:  ## Runs the OM docs in docker passing metamart-docs as volume for content and images
	docker pull metamart/docs-v1:latest
	docker run --entrypoint '/bin/sh' -v ${PWD}/metamart-docs/content:/docs/content/ -v ${PWD}/metamart-docs/images:/docs/public/images metamart/docs:latest -c 'yarn build'
...
snyk-ingestion-report:  ## Uses Snyk CLI to validate the ingestion code and container. Don't stop the execution
	...
	docker build -t metamart-ingestion:scan -f ingestion/Dockerfile.ci .
	snyk container test metamart-ingestion:scan --file=ingestion/Dockerfile.ci $(SNYK_ARGS) --json > security-report/ingestion-docker-scan.json | true;
...
snyk-server-report:  ## Uses Snyk CLI to validate the catalog code and container. Don't stop the execution
	@echo "Validating catalog container... Make sure the code is built and available under metamart-dist"
	docker build -t metamart-server:scan -f docker/development/Dockerfile .
	snyk container test metamart-server:scan --file=docker/development/Dockerfile $(SNYK_ARGS) --json > security-report/server-docker-scan.json | true;
...
snyk-ui-report:  ## Uses Snyk CLI to validate the UI dependencies. Don't stop the execution
	snyk test --file=metamart-ui/src/main/resources/ui/yarn.lock $(SNYK_ARGS) --json > security-report/ui-dep-scan.json | true;
...
snyk-ingestion-base-slim-report:
	...
	docker build -t metamart-ingestion-base-slim:scan -f ingestion/operators/docker/Dockerfile.ci --build-arg INGESTION_DEPENDENCY=slim .
	snyk container test metamart-ingestion-base-slim:scan --file=ingestion/operators/docker/Dockerfile.ci $(SNYK_ARGS) --json > security-report/ingestion-docker-base-slim-scan.json | true;
...
build-ingestion-base-local:  ## Builds the ingestion DEV docker operator with the local ingestion files
	...
	docker build -f ingestion/operators/docker/Dockerfile.ci . -t metamart/ingestion-base:local

.PHONY: build-ingestion-base-slim-local
build-ingestion-base-local:  ## Builds the ingestion DEV docker operator with the local ingestion files
	$(MAKE) install_dev generate
	docker build -f ingestion/operators/docker/Dockerfile.ci . -t metamart/ingestion-base-slim:local --build-arg INGESTION_DEPENDENCY=slim
...
update_pyproject_version:  ## To update the pyproject.toml files
	file_paths="ingestion/pyproject.toml \
				metamart-airflow-apis/pyproject.toml"; \
...
update_dockerfile_version:  ## To update the dockerfiles version
	@file_paths="docker/docker-compose-ingestion/docker-compose-ingestion.yml \
		     docker/docker-compose-metamart/docker-compose-metamart.yml \
