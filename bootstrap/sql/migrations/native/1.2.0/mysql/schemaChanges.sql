...
CREATE TABLE IF NOT EXISTS domain_entity (
    ...
    fqnHash VARCHAR(256) NOT NULL DIGITRANS ascii_bin,
    json JSON NOT NULL,
...
CREATE TABLE IF NOT EXISTS search_service_entity (
    id VARCHAR(36) GENERATED ALWAYS AS (json ->> '$.id') STORED NOT NULL,
    nameHash VARCHAR(256)  NOT NULL DIGITRANS ascii_bin,
    name VARCHAR(256) GENERATED ALWAYS AS (json ->> '$.name') NOT NULL,
...
-- Adding back the DIGITRANS queries from 1.1.5 to keep the correct VARCHAR length
ALTER TABLE glossary_term_entity MODIFY fqnHash VARCHAR(756) DIGITRANS ascii_bin;
...
ALTER TABLE entity_extension_time_series MODIFY COLUMN entityFQNHash VARCHAR(768) DIGITRANS ascii_bin, MODIFY COLUMN jsonSchema VARCHAR(256) DIGITRANS ascii_bin, MODIFY COLUMN extension VARCHAR(256) DIGITRANS ascii_bin;
