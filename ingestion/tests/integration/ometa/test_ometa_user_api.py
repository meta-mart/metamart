...
class OMetaUserTest(TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.team: Team = cls.metadata.create_or_update(
            data=CreateTeamRequest(
                teamType=TeamType.Group, name="ops.team", email="ops.team@trydigitrans.io"
            )
        )

        cls.user_1: User = cls.metadata.create_or_update(
            data=CreateUserRequest(
                name="random.user.es", email="random.user.es@trydigitrans.io"
            ),
        )

        cls.user_2: User = cls.metadata.create_or_update(
            data=CreateUserRequest(name="Levy", email="user2.1234@trydigitrans.io"),
        )

        cls.user_3: User = cls.metadata.create_or_update(
            data=CreateUserRequest(name="Lima", email="random.lima@trydigitrans.io"),
        )
    ...
    def test_es_search_from_email(self):
        ...
        self.assertIsNone(
            self.metadata.get_reference_by_email(email="idonotexist@trydigitrans.io")
        )

        # I can get User 1, who has the name equal to its email
        self.assertEqual(
            self.user_1.id,
            self.metadata.get_reference_by_email(email="random.user.es@trydigitrans.io")
            .root[0]
            .id,
        )
        ...
        self.assertEqual(
            self.user_2.id,
            self.metadata.get_reference_by_email(email="user2.1234@trydigitrans.io")
            .root[0]
            .id,
        )
        ...
        self.assertEqual(
            self.team.id,
            self.metadata.get_reference_by_email(email="ops.team@trydigitrans.io")
            .root[0]
            .id,
        )
