...
from _metamart_testutils.postgres.conftest import postgres_container
...
def patched_metadata(metadata, postgres_service, ingest_mysql_service, monkeypatch):
    ...
    monkeypatch.setattr(
        "metadata.ingestion.ometa.ometa_api.MetaMart.get_by_name",
        override_result_by_fqn(MetaMart.get_by_name),
    )

    monkeypatch.setattr(
        "metadata.ingestion.ometa.ometa_api.MetaMart.get_by_id",
        override_result_by_fqn(MetaMart.get_by_id),
    )
