...
class TestGreatExpectationIntegration(TestCase):
    ...
    server_config = MetaMartConnection(
        hostPort=WORKFLOW_CONFIG["metaMartServerConfig"]["hostPort"],
        authProvider=WORKFLOW_CONFIG["metaMartServerConfig"]["authProvider"],
        securityConfig=MetaMartJWTClientConfig(
            jwtToken=WORKFLOW_CONFIG["metaMartServerConfig"]["securityConfig"][
                "jwtToken"
            ]
        ),
    )  # type: ignore
