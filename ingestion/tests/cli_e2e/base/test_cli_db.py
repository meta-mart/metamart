...
from _metamart_testutils.pydantic.test_utils import assert_equal_pydantic_objects
...
class CliDBBase(TestCase):
    ...
    class TestSuite(TestCase, CliBase):  # pylint: disable=too-many-public-methods
        ...
        def test_data_quality(self) -> None:
            ...
            self.build_config_file()
            self.run_command()
            table: Table = self.metamart.get_by_name(
        ...
        def retrieve_table(self, table_name_fqn: str) -> Table:
            return self.metamart.get_by_name(entity=Table, fqn=table_name_fqn)

        def retrieve_sample_data(self, table_name_fqn: str) -> Table:
            table: Table = self.metamart.get_by_name(
                entity=Table, fqn=table_name_fqn
            )
            return self.metamart.get_sample_data(table=table)

        def retrieve_profile(self, table_fqn: str) -> Table:
            table: Table = self.metamart.get_latest_table_profile(fqn=table_fqn)

            return table

        def retrieve_lineage(self, entity_fqn: str) -> dict:
            return self.metamart.client.get(
