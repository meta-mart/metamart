...
class UsageQueryFilterTests(TestCase):
    ...
    @patch.object(MetaMart, "list_all_entities", mock_list_entities)
    def test_prepare_clickhouse(self):
        config = MetaMartWorkflowConfig.model_validate(mock_clickhouse_config)
        clickhouse_source = ClickhouseUsageSource.create(
            mock_clickhouse_config["source"],
            MetaMart(config.workflowConfig.metaMartServerConfig),
        )
