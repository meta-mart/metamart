...
class AtlasUnitTest(TestCase):
    ...
    def __init__(self, methodName, test_connection) -> None:
        ...
        self.config = MetaMartWorkflowConfig.model_validate(mock_atlas_config)
        self.atlas_source = AtlasSource.create(
            mock_atlas_config["source"],
            MetaMart(self.config.workflowConfig.metaMartServerConfig),
        )
        self.metadata = MetaMart(
            MetaMartConnection.model_validate(
                mock_atlas_config["workflowConfig"]["metaMartServerConfig"]
            )
        )
