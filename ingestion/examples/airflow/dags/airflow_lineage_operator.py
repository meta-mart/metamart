...
For this DAG to run properly we expected an MetaMart
Airflow connection named `metamart_conn_id`.
"""
...
DEFAULT_OM_AIRFLOW_CONNECTION = "metamart_conn_id"
...
# Create the default MetaMart Airflow Connection (if it does not exist)
res = requests.get(
...
with DAG(
    ...
    t4 = MetaMartLineageOperator(
        task_id="lineage_op",
        depends_on_past=False,
        server_config=MetaMartHook(DEFAULT_OM_AIRFLOW_CONNECTION).get_conn(),
